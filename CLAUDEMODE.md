...
- **Tech stack — RESOLVED, reversed 2026-08-26.** **Mobile app first** (`apps/mobile`, Expo SDK 57 + expo-router + React Native). `apps/web` **has been reduced to the admin panel** — the public pages were deleted 2026-08-28. Founder call: shopping belongs in an app, and an app carries more value than a site. I had argued the opposite earlier (marketing an unknown app is harder than sharing a link); the founder reaffirmed with their own reasoning, so the decision stands and is not to be re-litigated.
  ...
  - **What survived the 2026-08-28 deletion, and why.** Only four things. `/admin/*` is the panel itself. `/privacy` is a store requirement — both Apple and Google need a publicly reachable privacy-policy URL for the listing. `/auth/callback` and `/reset-password*` are where Supabase's emails land: the mobile app calls `signUp` without an `emailRedirectTo`, so confirmation links follow the project's Site URL to the web, and there is no forgot-password screen in the app at all. Deleting those two would have broken signup confirmation and left no way to reset a password. Replacing them with a `tallz://` deep link is the proper fix and is not done yet.
